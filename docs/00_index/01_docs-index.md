# MidesCloud → ERPNext: Мастер-индекс документации

**Статус:** ACTIVE  
**Обновлено:** 2026-05-26  
**Проект:** ERP для строительной компании МиДЭС на базе ERPNext

---

## Навигация по документам

### 📁 01_product — Продукт

| # | Документ | Назначение | Статус |
|---|----------|------------|--------|
| 01 | [product-vision.md](../01_product/01_product-vision.md) | Видение продукта и бизнес-контекст | ACTIVE |
| 02 | [module-mapping.md](../01_product/02_module-mapping.md) | Маппинг модулей MidesCloud → ERPNext | ACTIVE |

### 📁 02_research — Исследования

| # | Документ | Назначение | Статус |
|---|----------|------------|--------|
| 01 | [erpnext-capabilities.md](../02_research/01_erpnext-capabilities.md) | Возможности ERPNext для строительства | ACTIVE |
| 02 | [usability-evaluation.md](../02_research/02_usability-evaluation.md) | Анализ юзабилити и соответствия ERPNext бизнес-требованиям | ACTIVE |
| 03 | [procurement-workflow.md](../02_research/03_procurement-workflow.md) | Управление снабжением: пул заявок, распределение и сверка счетов | ACTIVE |
| 04 | [marketplace-apps-analysis.md](../02_research/04_marketplace-apps-analysis.md) | Анализ и выбор приложений маркетплейса | ACTIVE |
| 05 | [pre-mortem-analysis.md](../02_research/05_pre-mortem-analysis.md) | Pre-mortem анализ рисков и точек отказа сборки | ACTIVE |

### 📁 03_decisions — Решения

| # | Документ | Назначение | Статус |
|---|----------|------------|--------|
| 01 | [migrate-to-erpnext.md](../03_decisions/01_migrate-to-erpnext.md) | Решение о переходе на ERPNext | ACTIVE |
| 02 | [resolve-setup-blockers.md](../03_decisions/02_resolve-setup-blockers.md) | Разрешение блокировок при первичной настройке в Coolify | ACTIVE |
| 03 | [ui-ux-strategy.md](../03_decisions/03_ui-ux-strategy.md) | Стратегия упрощения UI/UX и интерфейсов ERPNext | ACTIVE |
| 04 | [marketplace-apps-selection.md](../03_decisions/04_marketplace-apps-selection.md) | Выбор и интеграция приложений маркетплейса (HR, Print Designer, Insights, S3) | ACTIVE |
| 05 | [branching-and-update-strategy.md](../03_decisions/05_branching-and-update-strategy.md) | Стратегия ветвления репозиториев и безопасного обновления ERPNext | ACTIVE |
| 06 | [switch-to-monolithic-docker.md](../03_decisions/06_switch-to-monolithic-docker.md) | Переход на монолитный Docker-образ для оптимизации ресурсов | ACTIVE |
| 07 | [optimize-docker-build-time.md](../03_decisions/07_optimize-docker-build-time.md) | Перенос сборки в Coolify и оптимизация кэширования слоев Docker | ACTIVE |
| 08 | [fix-assets-volume-and-build-args.md](../03_decisions/08_fix-assets-volume-and-build-args.md) | Решение проблемы с раздачей ассетов (CSS/JS) и долгой сборкой при смене переменных в Coolify | PROPOSED |

### 📁 04_architecture — Архитектура

| # | Документ | Назначение | Статус |
|---|----------|------------|--------|
| 01 | [system-overview.md](../04_architecture/01_system-overview.md) | Обзор архитектуры на ERPNext | ACTIVE |
| 02 | [docker-architecture.md](../04_architecture/02_docker-architecture.md) | Архитектура Docker-контейнеров и деплоя в Coolify | ACTIVE |

### 📁 06_delivery — Доставка

| # | Документ | Назначение | Статус |
|---|----------|------------|--------|
| 01 | [roadmap.md](../06_delivery/01_roadmap.md) | Дорожная карта внедрения | ACTIVE |
| 02 | [session-handover.md](../06_delivery/02_session-handover.md) | Передача контекста и опыт решения проблем в новую сессию | ACTIVE |

### 📁 08_reference — Справочники

| # | Документ | Назначение | Статус |
|---|----------|------------|--------|
| — | [old_docs/](../../old_docs/) | Старая документация MidesCloud v3 | ARCHIVED |

---

## Связанные ресурсы

- **Старый проект:** MidesCloud v3 (Next.js 14 + Supabase + Drizzle ORM)
- **Новая платформа:** ERPNext (Frappe Framework)
- **Репозиторий (старый):** `https://github.com/LepisevKalisey/MidesCloudv3`
